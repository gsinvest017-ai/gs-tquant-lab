...
- **M36** — notebook-discovery 的「跳過哪些目錄」收斂成單一 `_SKIP_DIR_PARTS`（修掉三支 discovery walk 只濾 `.ipynb_checkpoints` 的 under-discovery latent bug + 消掉第 4 份獨立 skip-set 定義）：M25 用行為 parity 鎖死了三支工具枚舉**同一組** `.ipynb`，但它們當時各自只 inline 檢查 `'.ipynb_checkpoints' in p.parts`——而 sync checker 的 `_orphan_py` 早就用更寬的 `_SKIP_DIR_PARTS`（`.git` / `.github` / `.ipynb_checkpoints` / `__pycache__` / `.venv` / `venv`）跳過非源碼目錄。這個不對稱是 latent bug：一個 bundled 在 `.venv/`（被安裝的套件附帶）或殘留在 `__pycache__` / `.git` 的 notebook 會被三支 discovery walk **全部誤抓並產生 spurious `.py`**，但 `_orphan_py` 又把它當成「不該存在的 `.py`」跳過——converter 產一個沒人管的 orphan、orphan 偵測卻看不見它。M36 依 M33（`convert_to_str` / `_is_magic_line` 單一來源）precedent，把 skip-set 抽成 converter 的 `_SKIP_DIR_PARTS` + `_in_skipped_dir(path)` 純函式（單一真理來源）；converter root-walk、sync `_pairs`、validator `_paired_py_files` 三支都改用同一 predicate，`_orphan_py` 改 import 同一常數（移除第 4 份獨立定義）。**provably 0-regen**：先 probe 確認全 repo 0 個 notebook 落在任一 skip dir 下（77 仍 77），收斂後 converter 重生 0 drift、sync 77/77、validator 77 OK。probe 另證舊 `.ipynb_checkpoints`-only filter 會誤抓 `.venv` / `venv` / `__pycache__` / `.git` 下的 bundled notebook（5 vs 1）。依 M25 行為-parity precedent 擴 `tools/tests/test_discovery_parity.py`：補 `SkipDirParityTests`（4 個 test，每 skip dir 放 root-level + nested notebook，斷言三支真實 discovery path 全跳過且三方一致、含逐 skip-dir `subTest` 與 negative-control raw-rglob 計數）+ `SkipDirConstantParityTests`（4 個 test，`assertIs` 鎖死 sync / validator 真的 reuse converter 的常數/predicate 而非各自重寫、釘死 canonical set、`.ipynb_checkpoints` 仍是子集）。動 converter（+1 常數、+1 helper、`main` 一行、docstring）＋ sync（import + `_pairs` 一行 + 刪本地 `_SKIP_DIR_PARTS`）＋ validator（import + `_paired_py_files` 一行）＋ M25 test 檔（+8 test、docstring）＋ README test-table 描述；不動 hook / CI workflow / `.gitattributes`，無 `.py` sibling regen。
- **M37** — `_orphan_py` 收斂改用共享 `_in_skipped_dir` predicate（消掉 sync checker 內**最後一份** inline skip-dir 掃描 + de-hardcode 測試裡手抄的 skip-set 字面值）：M36 把 skip-set 抽成 converter 的單一真理來源 `_SKIP_DIR_PARTS` + `_in_skipped_dir`，並讓 converter root-walk、sync `_pairs`、validator `_paired_py_files` 三支都改用同一 predicate——但 sync checker 的 `_orphan_py` 當時只改成 `import` 同一**常數**、迴圈內仍自己 inline 寫 `any(part in _SKIP_DIR_PARTS for part in py.parts)`，是 sync checker 內唯一沒收斂到 `_in_skipped_dir` 的第二份 skip 掃描（`_pairs` 已用 predicate）。同款隱患在測試端也有一份：`OrphanPyTests.test_skip_dirs_are_excluded` **硬抄**了 `('.git', '.github', '.ipynb_checkpoints', '__pycache__', '.venv', 'venv')` 這串字面值——M36 之後若有人往 `_SKIP_DIR_PARTS` 加一個新 skip dir（如 `.tox` / `node_modules`），production 會跳過它但這個 test 仍只驗舊六個，orphan-skip 覆蓋 silently 落後（test 綠但沒驗到新 dir）。M37 依 M33 / M36 單一來源 precedent 把這**兩份手抄**一次收掉：(1) production `_orphan_py` 的 skip 行改呼叫已 import 的 `_in_skipped_dir(py)`（`_SKIP_DIR_PARTS` 在 sync checker 內從此只剩 re-export，由 discovery parity 的 `assertIs` 守住、不再被 module body 直接掃）；(2) test `test_skip_dirs_are_excluded` 改 iterate 活的 `_SKIP_DIR_PARTS` 常數。**provably 零行為變動**：先 probe 15 條代表性路徑（root / 每個 skip dir 的 root-level + nested / handwritten / 一般）證 inline `any(...)` 與 `_in_skipped_dir` 逐一相等（0 mismatch），全 repo sync 77/77、validator 77 OK、orphan 0 不變。補 2 個 test 進 `OrphanPyTests`：`test_nested_skip_dirs_are_excluded`（skip dir 埋深層仍排除）、`test_orphan_skip_matches_in_skipped_dir`（**行為 parity**：在 root + 每個 skip dir 各放一個 orphan `.py`，斷言 `_orphan_py` 回傳的集合恰好 == `_in_skipped_dir` 不拒絕的那些，把 sync checker 兩個 skip 概念鎖到單一 predicate）。動 sync checker（`_orphan_py` 一行 + import 註解）＋ `test_check_ipynb_py_sync.py`（import +2 名、de-hardcode 1 test、+2 test）；**不動** converter / validator / hook / CI workflow / README / `.gitattributes`，無 `.py` sibling regen。
...
### M34 — string-aware magic-scan 行為 parity guard（兩支掃描迴圈鎖死）
- 為什麼補：M32（共用 `_advance_string_state`）+ M33（共用 `_is_magic_line`）已把「字串掃描器」與「文字判定」收斂成單一真理，但把兩者**組合起來**的那段 string-aware 掃描迴圈仍各寫各的：
  - converter `ipynb_to_py._sanitize_code` — `state=None` 起跑，逐行 `in_string = state is not None`，`not in_string and _is_magic_line(stripped)` → 註解掉，再 `state = _advance_string_state(ln, state)`
  - validator `check_converted_py._magic_check` — `state=None` 起跑，逐行 `state is None and _is_magic_line(line.lstrip())` → flag，再 `state = _advance_string_state(line, state)`
  兩個迴圈內嵌**同一條不變量**（「top-level 且 predicate 命中才算 magic」），但沒有任何東西強制它們對同一輸入標記**同一組行號**。日後重構任一迴圈的 gating（`in_string` 改在 advance 後算、`state is None` 放寬）就會 silently 漂移，且 CI 抓不到——production 中兩支永遠不看同一份原始文字（converter／sync checker 共用 `_sanitize_code`；validator 只掃已註解好的 `.py`，magic 早成 `# ...`）。這正是 M25 同款「三支獨立 walk 邏輯相同但沒被綁」的情境，只是換到 magic 掃描。
- 解法（依 M25 行為-parity precedent，非 text parsing）：新增 `tools/tests/test_magic_scan_parity.py`，在同一段原始碼上跑**兩支真實掃描迴圈**比對：
  - helper `_converter_commented(text)`：跑 `_sanitize_code`，逐行比對 input/output，差異行即 converter 註解掉的行；同時 `assert after == '# ' + before`——鎖死 converter 的唯一 transform 是 `'# '` 前綴（整個 parity 論證的前提），也防 helper 把別種 transform 誤判成註解。
  - helper `_validator_flagged(text)`：把**未註解**的原始碼寫進 temp `.py`、餵給 `_magic_check`（平常它掃已註解的產物會回 0；這裡故意餵原始碼，逼它的掃描迴圈去認出 converter 會註解的同一批 magic）。
  - `_assert_parity(text, expected)`：斷言 `converter == validator`，且兩者 == 釘死的 `expected`（後者擋住「兩支以相同方向一起壞、彼此仍相等」的 shared bug——光比 converter==validator 不夠）。
- 16 個 fixture：plain / trailing-`?`-comment（`x = run()  # done?` 不算 magic，M28 回歸）/ leading `!`／`%`／`%%`／`?` / indented `%time` / suffix-help `df.head?`／`obj??`（M33 能力）/ in-string magic 在 `"""` 與 `'''` docstring 內不算 / close 後真 magic（行 4）/ open 前真 magic（行 1）/ 單行 open-close 後下一行 magic（行 2）/ 綜合 torture（`{1, 6}`）/ helper-invariant（converter 永遠只 `'# '` 前綴）。
- 不動 production code：converter / validator / sync checker / hook（pre-commit／pre-push）/ CI workflow / `.gitattributes` 全部沒改。純測試新增 + README「## 測試」表補一列（否則 M21 的 `ReadmeTestTableParityTests` 會因 `glob('tools/tests/test_*.py')` 多一支而紅）。
- 不需動 CI workflow：`.github/workflows/ipynb-py-sync.yml` 已跑 `python3 -m unittest discover -s tools/tests`，16 個新 test 自動 pick up。
- 本地驗證：
  - `python3 tools/tests/test_magic_scan_parity.py -v` → `Ran 16 tests OK`
  - `python3 -m unittest discover -s tools/tests` → `Ran 253 tests OK`（237 + M34 16）
  - `python3 -m unittest tools.tests.test_readme.ReadmeTestTableParityTests` → 4 OK（新表列被接受）
  - `python3 tools/check_all.py` → All 4 steps passed、rc=0（77/77 in sync、0 orphan、0 leak）
...
### M36 — notebook-discovery dir-filtering 收斂成單一 `_SKIP_DIR_PARTS`
- 為什麼補：M25 用行為 parity 鎖死了三支工具枚舉**同一組** `.ipynb`，但當時三支 discovery walk 各自只 inline 檢查 `'.ipynb_checkpoints' in p.parts`，而 sync checker 的 `_orphan_py` 早就用更寬的 `_SKIP_DIR_PARTS`（`.git` / `.github` / `.ipynb_checkpoints` / `__pycache__` / `.venv` / `venv`）。這個不對稱是 latent bug：bundled 在 `.venv/`（安裝的套件附帶）或殘留在 `__pycache__` / `.git` 的 notebook 會被三支 discovery walk **全部誤抓並產生 spurious `.py`**，但 `_orphan_py` 又把那個 `.py` 當「不該存在」跳過——converter 產一個沒人管的 orphan、orphan 偵測卻看不見。這正是 M33（`_is_magic_line` 單一來源）同款「同一概念被獨立重寫多次」的情境，只是換到 skip-set。
- 解法（依 M33 單一真理來源 precedent）：把 skip-set 抽成 converter 的
  ```python
  _SKIP_DIR_PARTS = ('.git', '.github', '.ipynb_checkpoints', '__pycache__', '.venv', 'venv')
  def _in_skipped_dir(path): return any(part in _SKIP_DIR_PARTS for part in path.parts)
  ```
  三支 discovery walk（converter root-walk、sync `_pairs`、validator `_paired_py_files`）全改用 `_in_skipped_dir`；`_orphan_py` 改 `import` 同一常數，移除本地第 4 份定義。string-aware magic / orphan 各自的職責不變，純粹收斂「哪些目錄不是源碼樹」這個判定。
- **provably 0-regen**：先 probe 確認全 repo 0 個 notebook 落在任一 skip dir（`find` 空、converter dry-run 仍 77）→ 收斂後 `python3 tools/ipynb_to_py.py --dry-run .` = 77、sync 77/77、validator 77 OK、0 drift。
- Probe 實證 bug 存在（修前 vs 修後，同一 temp tree：root `real.ipynb` + `.venv`/`venv`/`__pycache__`/`.git` 各放一個 bundled `.ipynb`）：
  ```
  raw rglob sees:        5 (含 .venv/venv/__pycache__/.git bundled)
  OLD .ipynb_checkpoints-only filter would have seen: 5   ← bug
  sync _pairs / validator now see: ['real.ipynb']         ← 修後
  ```
- 依 M25 行為-parity precedent 擴 `tools/tests/test_discovery_parity.py`（8 → 16 test）：
  - `SkipDirParityTests`（4）— 每個 skip dir 放 root-level + nested notebook，斷言三支真實 discovery path（converter `--dry-run` 輸出 / `_pairs` / `_paired_py_files`）全跳過、且三方一致；含逐 skip-dir `subTest`（任一被誤刪會獨立紅）與 negative-control（raw rglob 必須看到 `2 + 2*len(_SKIP_DIR_PARTS)` 個，否則排除斷言 vacuous）
  - `SkipDirConstantParityTests`（4）— `assertIs` 鎖死 sync 的 `_SKIP_DIR_PARTS` **is** converter 的常數、validator 的 `_in_skipped_dir` **is** converter 的 predicate（證明真的 reuse 而非各自重寫）；釘死 canonical set；確認 `.ipynb_checkpoints` 仍是子集（pre-M36 行為保留）
- 影響面：動 converter（+1 常數 +1 helper + `main` 一行 + docstring）＋ sync（import + `_pairs` 一行 + 刪本地 `_SKIP_DIR_PARTS` + 註解）＋ validator（import + `_paired_py_files` 一行）＋ `test_discovery_parity.py`（+8 test + docstring）＋ README test-table 描述一列；**不動** hook（pre-commit / pre-push）/ CI workflow / `.gitattributes`，無 `.py` sibling regen。
- 已知限制（刻意不在本 milestone 處理）：`_in_skipped_dir` 比對絕對路徑的 `path.parts`，與既有 `_orphan_py` / pre-M36 checkpoint filter 行為一致——若 repo 本身落在某個名為 `venv` 的目錄下，整棵樹會被誤跳。這是既有慣例的 footgun（`_orphan_py` 早就這樣），非 M36 引入；要改需把比對改成 root-relative，屬未來 milestone。
- 本地驗證：
  - `python3 tools/tests/test_discovery_parity.py -v` → `Ran 16 tests OK`（M25 8 + M36 8）
  - `python3 -m unittest discover -s tools/tests` → `Ran 264 tests OK`（256 + M36 8）
  - `python3 -m unittest tools.tests.test_readme` → 29 OK（README test-table 描述更新被接受）
  - `python3 tools/check_all.py` → All 4 steps passed、rc=0（77/77 in sync、0 orphan、0 leak）

#### 用法
```bash
# 只跑 M36 新增的 parity guard
python3 -m unittest tools.tests.test_discovery_parity.SkipDirParityTests
python3 -m unittest tools.tests.test_discovery_parity.SkipDirConstantParityTests
```

### M37 — `_orphan_py` 收斂用共享 `_in_skipped_dir` predicate + de-hardcode 測試 skip-set
- 為什麼補：M36 把 skip-set 收斂成 converter 的單一真理來源（`_SKIP_DIR_PARTS` + `_in_skipped_dir`），讓 converter root-walk、sync `_pairs`、validator `_paired_py_files` 三支共用同一 predicate。但收斂留了兩條尾巴：
  1. **Production**：sync checker 的 `_orphan_py` 只改成 import 同一**常數**，迴圈裡仍自己 inline 寫 `any(part in _SKIP_DIR_PARTS for part in py.parts)`——是 sync checker 內**最後一份**沒走 `_in_skipped_dir` 的 skip 掃描（`_pairs` 已用 predicate）。第二份手抄正是這條 toolchain 一路在消滅的東西。
  2. **Test**：`OrphanPyTests.test_skip_dirs_are_excluded` 把 `('.git', '.github', '.ipynb_checkpoints', '__pycache__', '.venv', 'venv')` **硬抄**成字面值。M36 後若往 `_SKIP_DIR_PARTS` 加新 skip dir，production 會跳過它、但這個 test 仍只驗舊六個，orphan-skip 覆蓋 silently 落後。
- 改 `tools/check_ipynb_py_sync.py`：
  - `_orphan_py` 的 skip 行 `if any(part in _SKIP_DIR_PARTS for part in parts): continue` → `if _in_skipped_dir(py): continue`（`_in_skipped_dir` M36 已 import）；`parts = py.parts` 不再需要獨立綁定，handwritten 檢查改直接用 `py.parts`
  - 更新 import 區塊註解：點明 `_pairs` 與 `_orphan_py` **兩支**都呼叫 `_in_skipped_dir`、`_SKIP_DIR_PARTS` 從此只剩 re-export（由 discovery parity `assertIs` 守、不再被 module body 直接掃）
- 改 `tools/tests/test_check_ipynb_py_sync.py`：
  - import 補 `_SKIP_DIR_PARTS`（從 check_ipynb_py_sync，驗的是 sync checker 自己的 notion）與 `_in_skipped_dir`（從 ipynb_to_py）
  - `test_skip_dirs_are_excluded` 改 `for skip in _SKIP_DIR_PARTS`（de-hardcode，新增 skip dir 自動納入覆蓋）
  - 補 `test_nested_skip_dirs_are_excluded`：skip dir 埋在 `a/<skip>/deep/` 深層仍被排除（`_in_skipped_dir` 任意深度匹配）
  - 補 `test_orphan_skip_matches_in_skipped_dir`：**行為 parity** — root + 每個 skip dir 各放一個 orphan `.py`，斷言 `_orphan_py` 回傳集合恰好 == `_in_skipped_dir` 不拒絕的那些（+ sanity 斷言只剩 root orphan），把 sync checker 兩個 skip 概念鎖到單一 predicate，任一側日後漂移即紅
- **provably 零行為變動**：probe 15 條代表性路徑（root / 每個 skip dir 的 root-level + nested / handwritten `tools/` / 一般子目錄）證 inline `any(...)` 與 `_in_skipped_dir(py)` 逐一相等（**0 mismatch**）；`_in_skipped_dir(p) ≡ any(part in _SKIP_DIR_PARTS for part in p.parts)` 是定義式相等
- 沒動的東西：converter / validator / hook（pre-commit / pre-push）/ CI workflow / README / `.gitattributes` 全部沒改，無 `.py` sibling regen。M36 的 `SkipDirConstantParityTests.test_sync_reuses_converter_constant`（`check_ipynb_py_sync._SKIP_DIR_PARTS is ipynb_to_py._SKIP_DIR_PARTS`）仍綠，因為 import 行保留
- 本地驗證：
  - `python3 -m unittest tools.tests.test_check_ipynb_py_sync -v` → `Ran 36 tests OK`（含 de-hardcoded + 2 新 orphan parity test）
  - `python3 -m unittest discover -s tools/tests` → `Ran 266 tests OK`（M36 264 + M37 2）
  - `python3 tools/check_all.py` → All 4 steps passed、rc=0（77/77 in sync、0 orphan、0 leak）

#### 用法
```bash
# 只跑 M37 相關的 orphan skip-dir parity
python3 -m unittest tools.tests.test_check_ipynb_py_sync.OrphanPyTests.test_orphan_skip_matches_in_skipped_dir
python3 -m unittest tools.tests.test_check_ipynb_py_sync.OrphanPyTests
```

### Fallback 指引（接手 / rollback）
- M37 全部改動在三個檔：`tools/check_ipynb_py_sync.py`（`_orphan_py` 一行 + 註解）、`tools/tests/test_check_ipynb_py_sync.py`（import + 3 test）、本進度檔。
- 要 rollback 只 M37：`git revert <M37 commit>`（純加法 + 等價重寫，無資料/產物變動）。
- 驗收一條龍：`python3 tools/check_all.py`（== CI 四步）應 All 4 passed。
